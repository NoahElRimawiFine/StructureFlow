...
class Estimator:
    def __init__(
        ...
        if self.scale is None:
            self.scale = [
                [
                    ot.utils.euclidean_distances(
                        self.Xs[j][i] @ self.M_pca[j][:, : self.n_pca_components],
                        self.Xs[j][i + 1] @ self.M_pca[j][:, : self.n_pca_components],
                        squared=True,
                    ).mean()
                    for i in range(self.T - 1)
                ]
                for j in range(len(self.kos))
            ]
        self.A = (
            torch.zeros(
        ...
        self.us = [
            [
                torch.zeros(self.Xs[i][j].shape[0], dtype=torch.float64).to(self.device)
    ...
    def mask_matrix(self, ko):
        M = pd.DataFrame(
            np.ones((self.n_genes, self.n_genes), dtype=int),
            index=self.genes,
            columns=self.genes,
        )
        if ko is not None:
    ...
    def fit(self, print_iter=100, alg="joint", update_couplings_iter=100):
            ...
            for i in range(len(self.kos)):
                ...
                for j in range(self.T - 1):
                    ...
                    C = ot.utils.euclidean_distances(
                        ...
                        @ self.M_pca[i][:, : self.n_pca_components],
                        self.Xs[i][j + 1] @ self.M_pca[i][:, : self.n_pca_components],
                        squared=True,
                    )
        ...
        def R(A, b, t=0.0, include_diag=False):
            ...
            if not include_diag:
                M.fill_diagonal_(0)
            return t * (torch.sum((A * M) ** 2) + torch.sum(b) ** 2) + (1 - t) * (
                torch.sum(torch.abs(A * M)) + torch.sum(torch.abs(b))
            )
        ...
        for it in range(self.iter):
            ...
            if it % print_iter == 0:
                print(
                    f"iteration {it}, loss = {loss.item()}, L = {loss1.item()}, R = {loss2.item()}"
                )
