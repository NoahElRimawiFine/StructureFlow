...
def generate_linear_system(n_nodes, N_samples_in=500, T_timepoints=5, t1_end_time=2.5, dt_val=0.001, p_edge=0.25, rand_seed=42):
    effective_dt = dt_val 
    if n_nodes >= 500:
        effective_dt = 1e-5 
        print(f"Using smaller dt={effective_dt} for n={n_nodes} in generate_linear_system")
    elif n_nodes >= 100: 
        effective_dt = 1e-4 
        print(f"Using smaller dt={effective_dt} for n={n_nodes} in generate_linear_system")
    ...
    if max_real_eig_orig > 10: 
        print(f"Original max real eigenvalue ({max_real_eig_orig:.2f}) was large. Scaling A_true.")
        A_true = A_true_unscaled / stabilizing_shift 
    else:
        A_true = A_true_unscaled
    print(f"Max real eigenvalue of final A_true: {np.max(np.real(np.linalg.eigvals(A_true)))}")
        ...
        for i_tp in range(T_timepoints):
            ...
            while t_sim < ts[i_tp]:
                if not np.all(np.isfinite(x)):
                    print(f"Warning: Non-finite values in x before update at t_sim={t_sim}, i_tp={i_tp}. Clipping.")
                    x = np.clip(x, -1e8, 1e8)
                noise_term = np.random.randn(N_sim_samples, d_vars) @ noise_sigma_mat * effective_dt**0.5
                dynamics_term = -x @ A_mat.T * effective_dt
                x_new = x + dynamics_term + noise_term
                if not np.all(np.isfinite(x_new)):
                    print(f"Warning: Non-finite values in x_new after update at t_sim={t_sim}, i_tp={i_tp}. Clipping x_new.")
    ...
    ko_idx_list = []
    Ms_perturb = [np.ones((d_vars, d_vars),)]
    ko_label_list = ["wt",]
    ...
    for M_perturb_item in Ms_perturb:
        xs_simulated.append(simulate(A_true * M_perturb_item, int(N_samples_in // len(Ms_perturb))))
    xs_simulated = np.stack(xs_simulated)
    return xs_simulated, A_true, noise_sigma_mat

def train_random_discrete_time_ode(func, normalized_data, A_true_matrix, batch_size_ode, n_steps_per_transition=1000, plot_freq=1000, l1_reg=0.0, l2_reg=0.05, alpha_wass=0.1, plot=False, device=DEVICE, seed=42):
...
def jacobian_inference(A_estim_mat, y_true_vec):
    ...
    try:
        ...
        print(f"Could not calculate AUROC for Jacobian: {e}. y_true_vec may have only one class.")
        auroc_val = np.nan
...
def causal_graph_extraction(W_v_mat, y_true_vec):
    ...
    try:
        ...
        print(f"Could not calculate AUROC for causal graph: {e}. y_true_vec may have only one class.")
        auroc_val = np.nan
    return None, None, None, auroc_val
...
def _compute_jacobian_manually_chunked(func_to_diff, primary_input, chunk_size_cols=16):
    ...
    if output_y_raw.ndim == 0: 
        ...
        raise ValueError(f"func_to_diff output has unhandled shape: {output_y_raw.shape}. Expected effectively 1D.")
    n_in_dims = input_leaf.shape[0]
    jacobian_mat = torch.zeros((actual_n_out, n_in_dims), device=device, dtype=output_y_for_grad.dtype)
    for j_start in range(0, actual_n_out, chunk_size_cols):
        ...
        for k_abs_val in range(j_start, j_end): 
            v_basis = torch.zeros(actual_n_out, device=device, dtype=output_y_for_grad.dtype)
            v_basis[k_abs_val] = 1.0
            if input_leaf.grad is not None:
                input_leaf.grad.zero_() 
            (grad_input_val,) = torch.autograd.grad(outputs=output_y_for_grad, inputs=input_leaf, grad_outputs=v_basis, retain_graph=True)
            jacobian_mat[k_abs_val, :] = grad_input_val
        if DEVICE.type == 'cuda':
            torch.cuda.empty_cache()
    return jacobian_mat

def train_ngm_sf2m(A_true_matrix, xs_all_conditions, n_vars, sigma_matrix, N_samples_per_tp_gen=500, T_timepoints_gen=5, t1_end_time_gen=2.5, seed=42):
    torch.manual_seed(seed)
    np.random.seed(seed)
    ...
    optim_fm = torch.optim.AdamW(list(s_model_fm.parameters()) + list(v_model_fm.parameters()), 3e-3)
    xs_wt_unnormalized_np = xs_all_conditions[0] 
    num_timepoints_from_data = xs_wt_unnormalized_np.shape[0]
    grouped_data_wt = [torch.from_numpy(xs_wt_unnormalized_np[t_idx]).float() for t_idx in range(num_timepoints_from_data)]
    normalized_grouped_data_wt, _ = normalize_data(grouped_data_wt)
    normalized_cells_wt_all_times = torch.cat(normalized_grouped_data_wt, dim=0).to(DEVICE)
        ...
        def sample_map(self, pi_coupling, batch_size_val, replace=True):
            ...
            for attempt in range(3):
                try:
                    ...
                    p_flat = p_flat / p_flat.sum()
                    choices_idx = torch.multinomial(p_flat, num_samples=batch_size_val, replacement=replace)
                    choices_np_arr = choices_idx.detach().cpu().numpy()
                    pi_shape_1 = pi_coupling.shape[1] if isinstance(pi_coupling.shape[1], int) else pi_coupling.shape[1].cpu().item()
                    i_indices, j_indices = np.divmod(choices_np_arr, pi_shape_1)
                    return i_indices, j_indices
                except RuntimeError as e_runtime:
                    if "invalid multinomial distribution" in str(e_runtime) and attempt < 2:
                        min_clip_val = 1e-8 * (10**(attempt+1))
                        p_flat = torch.clip(p_flat, min=min_clip_val)
                        p_flat = p_flat / p_flat.sum()
                        print(f"Retrying sample_map with stronger stabilization: min_clip={min_clip_val}")
                    else:
                        print("Falling back to uniform sampling due to persistent numerical issues")
                        num_elements_p = p_flat.shape[0]
                        uniform_choices_idx = torch.randint(0, num_elements_p, (batch_size_val,), device=DEVICE)
                        choices_np_fallback = uniform_choices_idx.cpu().numpy()
                        i_indices_fb, j_indices_fb = np.divmod(choices_np_fallback, pi_shape_1)
                        return i_indices_fb, j_indices_fb
    def create_stable_entropic_otfm(x_samples_norm, n_dim, noise_mat, N_samples_tp, T_intervals, t1_total_time):
        ...
        if isinstance(scalar_sigma_fm, torch.Tensor):
            ...
            try:
                ...
                raise TypeError(f"Could not convert extracted sigma {scalar_sigma_fm} to float for EntropicOTFM")
        ot_model = fm.EntropicOTFM(x_samples_norm, torch.tile(torch.arange(T_intervals, device=DEVICE), (N_samples_tp, 1)).T.reshape(-1), dt=t1_total_time / T_intervals, sigma=scalar_sigma_fm, T=T_intervals, dim=n_dim, device=DEVICE)
        ...
        for i_interval in range(ot_model.T - 1):
            ...
            max_z_val_clip = 20.0
            x0_cpu_ot = np.nan_to_num(x0_cpu_ot, nan=0.0, posinf=max_z_val_clip, neginf=-max_z_val_clip)
            x1_cpu_ot = np.nan_to_num(x1_cpu_ot, nan=0.0, posinf=max_z_val_clip, neginf=-max_z_val_clip)
            ...
            if x0_cpu_ot.shape[0] == 0 or x1_cpu_ot.shape[0] == 0:
                print(f"Warning: Empty input arrays for Sinkhorn at time step {i_interval}. Using fallback uniform plan.")
                n_x0_fb_val = max(1, x0_cpu_ot.shape[0])
                n_x1_fb_val = max(1, x1_cpu_ot.shape[0])
                ot_model.Ts[i_interval] = torch.ones(n_x0_fb_val, n_x1_fb_val, device=DEVICE) / (n_x0_fb_val * n_x1_fb_val)
                continue
            C_cost_mat = ot.utils.euclidean_distances(x0_cpu_ot, x1_cpu_ot, squared=True) / 2
            C_cost_mat = np.nan_to_num(C_cost_mat, nan=0.0, posinf=1e6, neginf=0.0) 
            C_cost_mat = np.clip(C_cost_mat, 0, 1e6) 
            p_marginal_np = (torch.ones(x0_cpu_ot.shape[0], dtype=torch.float64) / x0_cpu_ot.shape[0]).numpy()
            q_marginal_np = (torch.ones(x1_cpu_ot.shape[0], dtype=torch.float64) / x1_cpu_ot.shape[0]).numpy()
            C_cost_np_float64 = C_cost_mat.astype(np.float64)
            eps_reg = epsilon_scale_val * ot_model.dt * ot_model.sigma**2
            try:
                plan_ot = ot.sinkhorn(p_marginal_np, q_marginal_np, C_cost_np_float64, eps_reg, method="sinkhorn_stabilized", numItermax=2000, stopThr=1e-5)
                ot_model.Ts[i_interval] = torch.tensor(plan_ot, device=DEVICE, dtype=torch.float32)
            except Exception as e_sinkhorn:
                print(f"Sinkhorn failed at time {i_interval}, using approximate plan: {e_sinkhorn}")
                n_x0_err, n_x1_err = x0_ot.shape[0], x1_ot.shape[0]
                ot_model.Ts[i_interval] = torch.ones(n_x0_err, n_x1_err, device=DEVICE) / (n_x0_err * n_x1_err)
        return ot_model
    print("Generating stabilized entropic OT model...")
    otfm_model = create_stable_entropic_otfm(normalized_cells_wt_all_times, n_vars, sigma_matrix, N_samples_per_tp_gen, T_timepoints_gen, t1_end_time_gen)
    ...
    for i_step in tqdm(range(training_steps_fm)):
        try:
            _x_batch, _s_batch, _u_batch, _t_batch, _t_orig_batch = otfm_model.sample_bridges_flows(batch_size=batch_size_fm)
            _x_batch, _s_batch, _u_batch, _t_batch, _t_orig_batch = (_val.to(DEVICE) for _val in [_x_batch, _s_batch, _u_batch, _t_batch, _t_orig_batch])
            if torch.isnan(_x_batch).any() or torch.isnan(_s_batch).any() or torch.isnan(_u_batch).any() or torch.isinf(_x_batch).any():
                print(f"Warning: NaN/Inf values detected at iteration {i_step}. Skipping this batch.")
                continue
            optim_fm.zero_grad()
            s_input_fm = _x_batch.unsqueeze(1)
            v_input_fm = _x_batch.unsqueeze(1)
            s_fit_fm = s_model_fm(_t_batch, _x_batch).squeeze(1)
            v_fit_fm = v_model_fm(_t_batch, v_input_fm).squeeze(1) - otfm_model.sigma**2 / 2 * s_model_fm(_t_batch, _x_batch)
            L_score_fm = torch.mean((_t_orig_batch * (1 - _t_orig_batch)) * (s_fit_fm - _s_batch) ** 2)
            L_flow_fm = torch.mean((v_fit_fm * otfm_model.dt - _u_batch) ** 2)
            L_reg_fm = v_model_fm.l2_reg() + v_model_fm.fc1_reg()
            L_total_fm = alpha_fm * L_score_fm + (1 - alpha_fm) * L_flow_fm + reg_fm * L_reg_fm
            if i_step % 500 == 0: print(L_score_fm.item(), L_flow_fm.item(), L_reg_fm.item())
            trace_loss.append(L_total_fm.item())
            L_total_fm.backward()
            optim_fm.step()
            proximal(v_model_fm.fc1.weight, v_model_fm.dims, lam=v_model_fm.GL_reg, eta=0.01)
        except RuntimeError as e_runtime_train:
            if "invalid multinomial distribution" in str(e_runtime_train):
                print(f"Numerical instability (multinomial) at iter {i_step}. Reducing batch size.")
    ...
    for i_time_jac in range(otfm_model.T):
        ...
        samples_at_t_current_jac = otfm_model.x[otfm_model.t_idx == i_time_jac, :].to(DEVICE)
        if samples_at_t_current_jac.shape[0] == 0:
            print(f"Warning: No samples found for time index {i_time_jac} in Jacobian calculation. Skipping.")
            continue
        J_sum_for_current_t_fm = torch.zeros((n_vars,n_vars), device=DEVICE, dtype=torch.float32)
        ...
        for sample_idx_jac in range(samples_at_t_current_jac.shape[0]):
            ...
            J_single_sample_fm = _compute_jacobian_manually_chunked(func_to_diff_fm, x_sample_jac, chunk_size_cols=max(1, n_vars // 32 if n_vars > 32 else 1))
            J_sum_for_current_t_fm += J_single_sample_fm
            num_samples_processed_for_t_fm += 1
        if num_samples_processed_for_t_fm > 0:
            J_mean_for_current_t_fm = J_sum_for_current_t_fm / num_samples_processed_for_t_fm
            J_list_time_means_fm.append(J_mean_for_current_t_fm)
        else:
            print(f"Warning: No Jacobians computed for time index {i_time_jac} despite having samples. Skipping.")
    if not J_list_time_means_fm:
        print("Warning: No Jacobians computed across any time points. Returning zero matrix for A_estim.")
    ...
    return jacobian_derived_interaction_matrix, direct_fc1_causal_graph, y_true_flat_fm, pure_training_elapsed_time

def run_experiment_for_n(n_val_in, seed_val):
    print(f"\nRunning experiment for n = {n_val_in}, seed = {seed_val}...")
    xs_raw_data, A_true_data, noise_matrix_sigma_data = generate_linear_system(n_val_in, rand_seed=seed_val)
    print(f"Shape of xs_raw before cleaning: {xs_raw_data.shape if xs_raw_data is not None else 'None'}")
    xs_finite_data = np.nan_to_num(xs_raw_data, nan=0.0, posinf=1e6, neginf=-1e6)
    xs_clipped_data = np.clip(xs_finite_data, -1e3, 1e3)
    print(f"Data range in xs_clipped: min={np.min(xs_clipped_data)}, max={np.max(xs_clipped_data)}")
    ...
    if n_val_in <= 100:
        ...
        grouped_data_node_list = [torch.from_numpy(xs_wt_node_processed_data[t_idx]).float() for t_idx in range(xs_wt_node_processed_data.shape[0])]
        normalized_data_node_list, _ = normalize_data(grouped_data_node_list)
        
        n_steps = 1000 
        batch_size_ode_for_n = 64
        
        _, W_v_disc_val, y_true_disc_val, node_ode_integration_time_specific = train_random_discrete_time_ode(
            func_node_model, normalized_data_node_list, A_true_data, 
            batch_size_ode=batch_size_ode_for_n,
            n_steps_per_transition=n_steps, device=DEVICE, seed=seed_val
        )
        
        node_total_time = time.time() - node_start_time
        print(f"Total NGM-NODE block time: {node_total_time:.2f}s")
        print(f"NGM-NODE ODE integration loop time: {node_ode_integration_time_specific:.2f}s")
        
        _, _, _, auroc_disc_causal_val = causal_graph_extraction(W_v_disc_val, y_true_disc_val)
    else:
    ...
    _, _, _, auroc_ngm_causal_val = causal_graph_extraction(direct_graph_ngm_val, y_true_ngm_val)
...
def main():
    ...
    summary_filename_out = f"results_summary_n{n_val_run}_seed_{experiment_seed_run}.txt"
    summary_lines_list = [
    ...
    for metric_key in metrics:
        if metric_key in result_dict_run:
            ...
            val_str_display = f"{metric_val:.3f}" if isinstance(metric_val, (float, int)) and not np.isnan(metric_val) else "N/A"
